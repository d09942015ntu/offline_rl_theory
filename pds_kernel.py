...
def pevi_kernel_approx(Dtheta, env, B, lamda):
    H = env.H
    Aspace = env.A
    #flat_data = []
    #for h_ in range(H):
    ...
    for h in reversed(range(H)):
        ...
        def var_kernel_sample(z):
            k_array = np.array([env.kernel(z,zi) for zi in Zh])
            return (lamda**0.5)*((env.kernel(z,z) - np.dot(np.dot(lambda_inv,k_array),k_array))**0.5)
...
def data_sharing_kernel_approx(D1, D2,
    ...
    pi_hat = pevi_kernel_approx(Dtheta, env, B, lamda)
...
def run_default_settings():
    ...
    env = EnvKernel(H=H, seed=0, s_size=5)
    run_experiment(H, env, delta, B, nu, lamda, beta_h_func, output_name="kernel_env1")
...
def run_debug():
    ...
    R1 = evaluate(env=env, pi_func=pi_hat)
    print(f"N1={N1}, N2={N2}, R1={R1}, Rrand={Rrand}")
